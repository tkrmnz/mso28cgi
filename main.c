...
//		fprintf(stderr,"OK! directory: %s\n", dir);
		...
		{//search for MSO-28
			...
			if(strlen(sdir)>6)
			{
				...
				{
				newlen = strlen(sdirnm) + strlen(sdir) + 2;
				sdirnm = (char *) malloc(newlen);
				line = (char *) malloc(newlen);
				strcat(sdirnm,"/dev/");
				strcat(sdirnm,sdir);
				strcpy(line,sdirnm);
//				printf("%s %s\n",line,sdirnm);
...
//		BytesToRead = res;
        ...
        {
            ii = bbb[(Cnt * 4)];              //dso 1
            jj = (bbb[(Cnt * 4) + 1] & 0x03) << 8; //dso1

            kk = (bbb[(Cnt * 4) + 1] & 0xfc) >> 2; //dso2
            mm = (bbb[(Cnt * 4) + 2] & 0x0f) << 6; //dso2

            nn = (bbb[(Cnt * 4) + 2] & 0xf0) >> 4; //la
            pp = (bbb[(Cnt * 4) + 3] & 0x0f) << 4; //la

            AnalogDataA[Cnt - AdjAddr] = jj | ii;
            AnalogDataB[Cnt - AdjAddr] = mm | kk;
            LogicData[Cnt - AdjAddr] = nn | pp;
        }
...
//			&& strcmp(name_val_pairs[nv_entry_number].value,"r")==0){
//			printf("Ready\n\n");	
...
//	PrintMSOSettings();
	...
	//	InitSettings();	//clear internal settings variable
